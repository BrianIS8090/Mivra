...
- **IPC-команды** (`src-tauri/src/commands.rs`): `s3_set_secret`, `s3_clear_secret`, `s3_secret_exists`, `s3_test_connection`, `s3_upload_file`, `s3_upload_bytes`. Secret НИКОГДА не возвращается на фронт — только через keyring.
...
- **Frontend:**
  - `src/hooks/useS3Upload.ts` — основной хук с `ready` (s3 настроен И secret в keyring), `uploadAndInsertBytes` (paste/clipboard), `uploadAndInsertFile` (drag&drop/file picker). Whitelist: картинки + pdf/zip/mp4/webm. Hard-limit 100 MB, soft-warn confirm на >10 MB.
  - `src/components/S3Settings/S3SettingsDialog.tsx` — форма настроек, открывается из Toolbar.
  - `src/components/Editor/Editor.tsx` — drag&drop через `getCurrentWindow().onDragDropEvent`, paste через `document.addEventListener('paste', ..., true)` (capture-phase, до Crepe).
  - `src/hooks/useMarkdownActions.ts` — `insertAssetAction` ветвится: если S3 настроен → file dialog → upload, иначе → старый `pickAndFormatAsset` (локальный assets/).
...
This project is indexed by GitNexus as **Mivra** (329 symbols, 762 relationships, 22 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
